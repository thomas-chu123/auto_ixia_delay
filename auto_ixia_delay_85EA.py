#import Utils_8_5EA.IxLoadUtils as IxLoadUtils
#import Utils_8_5EA.IxRestUtils as IxRestUtils
...
class Ixload_Test(Thread):
    ...
    def ixia_start(self):
        ...
        self.connection = IxRestUtils.getConnection(kGatewayServer, kGatewayPort, httpRedirect=kHttpRedirect)
        self.connection.setApiKey(kApiKey)
        self.sessionUrl = None
        # create a session
        try:
            IxLoadUtils.log("Creating a new session...")
            self.ui_object.update_output("Creating a new session...")
            self.sessionUrl = IxLoadUtils.createSession(self.connection, kIxLoadVersion)
            ...
            IxLoadUtils.addChassisList(self.connection, self.sessionUrl, kChassisList)
